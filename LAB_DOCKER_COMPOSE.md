...
```คือการแบ่งขั้นตอนการสร้าง Image ออกเป็นหลายส่วนใน Dockerfile เดียวครับ
**Stage แรก:** ใช้ Image ตัวใหญ่ที่มีเครื่องมือครบเพื่อ **Build/Compile** โค้ด
**Stage สุดท้าย:** ก๊อปปี้มาเฉพาะ **"ไฟล์ที่ใช้งานจริง"** ไปวางใน Image ตัวเล็ก (เช่น Alpine) แล้วทิ้งเครื่องมือที่ใช้ Build ทั้งหมดไป
...
```
-e DB_PATH=/app/data/booking.db (Environment Variable)

คือการตั้งค่าตัวแปร: ส่งค่า DB_PATH เข้าไปให้โปรแกรมข้างใน Container รู้ว่าไฟล์ Database อยู่ที่ไหน (เหมือนการตั้งค่า Config)

-v $(pwd)/test-data:/app/data (Volume Mount)

คือการเชื่อมโฟลเดอร์: เอาโฟลเดอร์ test-data ในเครื่องเรา (ตำแหน่งปัจจุบัน) ไปแปะทับโฟลเดอร์ /app/data ใน Container

ประโยชน์: ทำให้ไฟล์ที่เกิดขึ้นใน Container ไม่หายไปเมื่อลบ Container ทิ้ง และเราสามารถแก้ไฟล์จากเครื่องตัวเองได้ทันทีครับ
```
...
1. `booking-backend:1.0` คืออะไร?
   ...
   > _คำตอบ_:booking-backend:1.0 คือ Docker Image ที่เราระบุเพื่อนำมาสร้าง Container โดยแบ่งเป็น 2 ส่วนครับ:booking-backend (Image Name): คือชื่อของ Image (ในที่นี้คือแอปฯ Backend สำหรับระบบจอง)1.0 (Tag): คือ "ป้ายกำกับ" เพื่อบอก เวอร์ชัน ของ Image นั้นๆ

2. ถ้าต้องการให้ ทดสอบ API ด้วย `curl http://localhost` ต้องทำอย่างไร?

   > _คำตอบ_: เปิด Terminal ใหม่แล้วพิมพ์คำสั่งได้เลย:
   Bash
   curl http://localhost
---
...
1. ขนาดของ `booking-frontend:1.0` เทียบกับ `booking-backend:1.0` ต่างกันอย่างไร? เพราะเหตุใด?
   ...
   > _คำตอบ_: Frontend เล็กกว่า เพราะมีแค่ "ไฟล์หน้าเว็บ" (HTML/JS) ที่ถูกบีบอัดแล้ว รันบน Web Server เบาๆ อย่าง Nginx
  Backend ใหญ่กว่า เพราะต้องแบก "เครื่องยนต์" (Runtime เช่น Node.js/Python) และ Library ต่างๆ เพื่อใช้ประมวลผลครับ

2. `location /api/` ใน nginx.conf ทำหน้าที่อะไร?

   > _คำตอบ_: location /api/ ใน Nginxเป็น "ตัวดักจับ URL" ที่ขึ้นต้นด้วย /api/ เพื่อส่งต่อไปหาเครื่อง Backend โดยเฉพาะครับ

3. ทำไม Frontend Container ถึงใช้ชื่อ `backend` ในการ Proxy ได้?

   > _คำตอบ_: เพราะมีการใช้ **Docker Network** ครับ
**Service Discovery:** Docker มีระบบ DNS ภายในที่ช่วยให้ Container คุยกันผ่าน **"ชื่อ Container"** ได้เลย**การทำงาน:** เมื่อคุณตั้งชื่อ Container ว่า `backend` ตัว Nginx (Frontend) จะมองเห็นชื่อนั้นเป็น IP Address โดยอัตโนมัติ**ข้อดี:** ไม่ต้องระบุ IP จริงของเครื่อง ทำให้ย้ายเครื่องหรือรันใหม่ได้ง่ายโดยไม่ต้องแก้ Code ครับ
---
